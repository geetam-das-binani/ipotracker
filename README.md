...
## All cron jobs run via GitHub Actions, not Vercel Cron

All five refresh jobs (`subscriptions`, `grey-market`, `new-issues`,
`issue-details`, `listing`) run as GitHub Actions scheduled
workflows in `.github/workflows/`, each one just `curl`-ing the
corresponding `/api/cron/*` endpoint on the deployed backend.
`backend/vercel.json` carries no `crons` config at all — Vercel Cron
isn't used anywhere in this project.

This was a deliberate simplification: it was originally split across
both Vercel Cron (for the two genuinely-once-daily jobs, which fit
Vercel's Hobby-plan limit) and GitHub Actions (for the three more
frequent ones, which don't). Moving all five to GitHub Actions
removes that split — one mechanism, no dependency on Vercel's plan
tier, and every job's schedule lives in one place
(`.github/workflows/`) instead of two.

### Setting it up

1. Push this repo to GitHub (required — Actions only run from a
   GitHub-hosted repo).
2. In the repo: **Settings → Secrets and variables → Actions → New
   repository secret**, add two:
   - `BACKEND_URL` — the deployed backend's URL, no trailing slash
     (e.g. `https://ipo-tracker-backend.vercel.app`)
   - `CRON_SECRET` — the same value set in the backend's `CRON_SECRET`
     env var
3. Nothing else to configure — each workflow file already points at
   the right endpoint and schedule (converted from the IST windows in
   §5 of the spec to UTC, since GitHub Actions cron, like Vercel's,
   is always UTC).
4. Verify: **Actions tab → pick a workflow → Run workflow** (the
   `workflow_dispatch` trigger in every file enables this manual
   button) to fire it once immediately rather than waiting for the
   real schedule, and confirm it returns success rather than a curl
   error.

**Known GitHub Actions caveats, stated honestly**: scheduled
workflows can run a few minutes late under GitHub's own load (not
exact-to-the-minute the way a dedicated cron daemon is), and GitHub
automatically **disables scheduled workflows on a repo after 60 days
with no commits** — if the repo goes quiet for two months, the
workflows need to be re-enabled manually from the Actions tab.
